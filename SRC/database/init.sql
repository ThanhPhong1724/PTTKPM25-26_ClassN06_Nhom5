...
-- Insert Categories với UUID được generate
DO $$
DECLARE
    cat1_id uuid;
    cat2_id uuid;
...
BEGIN
    ...
    VALUES
    (uuid_generate_v4(), 'Bánh kem truyền thống', 'Các loại bánh kem cổ điển với hương vị truyền thống', NULL, NOW(), NOW())
    RETURNING id INTO cat1_id;

    INSERT INTO categories (id, name, description, img, "createdAt", "updatedAt")
    VALUES
    (uuid_generate_v4(), 'Bánh kem tình yêu', 'Bánh kem dành cho các dịp kỷ niệm tình yêu', NULL, NOW(), NOW())
    RETURNING id INTO cat2_id;

    INSERT INTO categories (id, name, description, img, "createdAt", "updatedAt")
    VALUES
    (uuid_generate_v4(), 'Bánh mousse', 'Bánh mousse với các hương vị độc đáo', NULL, NOW(), NOW())
