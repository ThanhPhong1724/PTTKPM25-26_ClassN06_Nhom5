-- Migration: Custom Cake Options
-- Purpose: Support custom cake builder feature
-- Date: 26/10/2025

-- Create cake_options table
CREATE TABLE IF NOT EXISTS cake_options (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  category VARCHAR(50) NOT NULL, -- 'size', 'cake_base', 'frosting', 'decoration', 'flavor'
  name VARCHAR(100) NOT NULL,
  description TEXT,
  price DECIMAL(10, 2) NOT NULL DEFAULT 0, -- Base price or additional cost
  is_default BOOLEAN DEFAULT false, -- One option per category should be default
...
-- Add comments
COMMENT ON TABLE cake_options IS 'Stores all available options for custom cake builder';
COMMENT ON COLUMN cake_options.category IS 'Option category: size, cake_base, frosting, flavor, decoration';
COMMENT ON COLUMN cake_options.price IS 'Additional price for this option (0 for default options)';
COMMENT ON COLUMN cake_options.is_default IS 'Whether this is the default option for its category';
COMMENT ON COLUMN cake_options.display_order IS 'Order to display options in UI';
